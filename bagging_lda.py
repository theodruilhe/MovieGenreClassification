...
def bagging_discriminant_analysis(pca_df, n_estimators=10, test_size=0.2, random_state=42):
    ...
    pca_df_train, pca_df_test = train_test_split(pca_df, test_size=test_size, random_state=random_state)
    ...
    full_transformed_data_test = pd.concat([y_test.reset_index(drop=True), full_transformed_data_test], axis=1)

    # Find the most common prediction across all LDAs
    full_transformed_data_test['pred_maj_vote'] = full_transformed_data_test.iloc[:, 1:].mode(axis=1)[0]
